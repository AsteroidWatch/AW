...
public class TestSizeComparison extends SimpleApplication {
	...
	ArrayList<Asteroid> getAsteroids(int count) {
		...
		for (int i = 1; i < count + 1; i++) {
			Spatial spatial = assetManager.loadModel("Assets/asteroid" + i + 5 + ".obj");
	...
	void addEventsToCinematic(int period) {
		...
		for (MotionEvent motionEvent : moveEvents) {			
			cinematic.addCinematicEvent(0.1f + time + 3 * i, motionEvent);
			i++;
		}
	} 
	 	
	ArrayList<MotionEvent> getMoveEvents(int period) {
	...
	ArrayList<Asteroid> getRelevantAsteroids() {
	...
	ArrayList<Vector3f> getWayPoints() {
		...
		for (Asteroid asteroid : getRelevantAsteroids()) {
			wayPoints.add(asteroid.model.getLocalTranslation().add(0, 0, asteroid.model.getWorldScale().z * 5));
